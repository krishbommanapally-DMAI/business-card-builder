      ...
      >
        ...
        <div className="flex flex-col items-center -mt-12 px-6 pb-4 shrink-0 text-center relative z-10">
          <img 
            ...
            alt="profile logo"
          />
          <h2 className="text-xl font-display font-extrabold mt-3 tracking-tight">
            {card.profile.prefix && `${card.profile.prefix} `}{card.profile.firstName} {card.profile.lastName}
          </h2>
          <p className="text-xs text-slate-500 font-semibold mt-0.5">{card.profile.designation}</p>
          <p className="text-sm font-extrabold mt-0.5 tracking-tight" style={{ color: card.theme.primaryColor }}>
            {card.profile.company}
          </p>
          <p className="text-xs text-slate-400 mt-3 italic px-4 leading-normal max-w-sm">
        ...
        <div className="grid grid-cols-4 gap-2 px-6 shrink-0 mb-6">
          {card.contact.phone && (
            ...
            >
              <Phone size={16} className="text-slate-500" style={{ color: card.theme.primaryColor }} />
          ...
          {card.contact.email && (
            <a 
              ...
              className="bg-slate-50 dark:bg-white/5 border dark:border-white/5 py-3 rounded-2xl flex flex-col items-center gap-1.5 text-center hover:bg-slate-100 dark:hover:bg-white/10 transition-all cursor-pointer"
            >
              <Mail size={16} className="text-slate-500" style={{ color: card.theme.primaryColor }} />
          ...
          <button 
            ...
            className="bg-slate-50 dark:bg-white/5 border dark:border-white/5 py-3 rounded-2xl flex flex-col items-center gap-1.5 text-center hover:bg-slate-100 dark:hover:bg-white/10 transition-all cursor-pointer"
          >
            <Download size={16} className="text-slate-500" style={{ color: card.theme.primaryColor }} />
            <span className="text-[10px] font-bold text-slate-600 dark:text-slate-300">Save vCard</span>
          </button>
        </div>

        {/* CORE DETAILS COLLAPSIBLE DESCRIPTION */}
        <div className="px-6 py-4 border-t border-slate-100 dark:border-white/5">
          <span className="text-[10px] font-extrabold text-slate-400 uppercase tracking-widest">Biography</span>
        ...
        {card.services.length > 0 && (
          <div className="px-6 py-4 border-t border-slate-100 dark:border-white/5 flex flex-col gap-3">
            <span className="text-[10px] font-extrabold text-slate-400 uppercase tracking-widest">My Services</span>
            <div className="flex flex-col gap-2.5">
              {card.services.map((s) => (
                <div key={s.id} className="p-3 rounded-2xl bg-slate-50 dark:bg-white/5 border dark:border-white/5 hover:shadow-sm transition-all">
                  <h4 className="font-bold text-xs text-slate-900 dark:text-white flex items-center gap-1.5">
                    <Sparkles size={14} style={{ color: card.theme.primaryColor }} /> {s.title}
                  </h4>
        ...
        {card.products.length > 0 && (
          <div className="px-6 py-4 border-t border-slate-100 dark:border-white/5 flex flex-col gap-3">
            <span className="text-[10px] font-extrabold text-slate-400 uppercase tracking-widest">Store Products</span>
                ...
                <div key={p.id} className="p-4 rounded-2xl bg-slate-50 dark:bg-white/5 border dark:border-white/5 flex flex-col justify-between hover:shadow-sm transition-all gap-3">
                  <div>
                    <div className="flex items-center justify-between gap-4">
                      ...
                      <span className="text-xs font-black text-indigo-600 dark:text-indigo-400 shrink-0 bg-white dark:bg-white/10 px-2 py-1 rounded border dark:border-white/5">{p.price}</span>
                    </div>
                    <p className="text-slate-500 dark:text-slate-400 text-xs mt-1.5 leading-normal font-medium">
                      {p.description}
                    </p>
                  </div>
                  ...
                  {p.whatsappOrder && (
                    <button 
                      ...
                      className="w-full bg-emerald-600 hover:bg-emerald-700 text-white font-bold py-2 rounded-xl text-xs flex items-center justify-center gap-1.5 shadow-md shadow-emerald-600/10 active:scale-98 transition-all cursor-pointer"
                    >
                      <ShoppingBag size={14} /> Buy via WhatsApp
                    </button>
                  )}
                </div>
              ))}
        ...
        {card.skills.length > 0 && (
          <div className="px-6 py-4 border-t border-slate-100 dark:border-white/5 flex flex-col gap-3">
            <span className="text-[10px] font-extrabold text-slate-400 uppercase tracking-widest">Technical Deliverables</span>
        ...
        {card.socialLinks.length > 0 && (
          <div className="px-6 py-4 border-t border-slate-100 dark:border-white/5 flex flex-col gap-3">
            <span className="text-[10px] font-extrabold text-slate-400 uppercase tracking-widest">Find Me Elsewhere</span>
            <div className="grid grid-cols-2 gap-2.5">
              {card.socialLinks.map((link) => (
                <a 
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="p-3 bg-slate-50 dark:bg-white/5 border dark:border-white/5 rounded-2xl flex items-center gap-2.5 hover:bg-slate-100 dark:hover:bg-white/10 transition-all font-bold text-xs"
                >
        ...
        <div className="px-6 py-4 border-t border-slate-100 dark:border-white/5 flex flex-col gap-3">
          <span className="text-[10px] font-extrabold text-slate-400 uppercase tracking-widest">Appointment Slots</span>
          <p className="text-[11px] text-slate-500 font-medium">Select a time slot below to secure your 15-minute consultation.</p>
          ...
          <div className="grid grid-cols-2 gap-2.5">
            {mockTimeSlots.map((slot) => (
              <button
                ...
                className={`p-3 text-xs font-bold border rounded-2xl transition-all cursor-pointer ${!slot.available ? 'bg-slate-100 dark:bg-white/5 border-slate-200/50 text-slate-400 line-through cursor-not-allowed' : selectedSlot === slot.time ? 'border-indigo-600 bg-indigo-50/50 text-indigo-950' : 'bg-slate-50 dark:bg-white/5 border-transparent text-slate-700 dark:text-slate-300 hover:border-slate-300'}`}
              >
                {slot.time}
              </button>
            ))}
        ...
        {card.contact.address && (
          <div className="px-6 py-4 border-t border-slate-100 dark:border-white/5 flex flex-col gap-3">
            <span className="text-[10px] font-extrabold text-slate-400 uppercase tracking-widest">Office Location</span>
            <div className="bg-slate-50 dark:bg-white/5 border dark:border-white/5 rounded-2xl p-4 flex gap-3.5 items-start">
              ...
              <div>
                ...
                <a 
                  ...
                  target="_blank" 
                  rel="noreferrer" 
                  className="text-[10px] font-bold text-indigo-600 dark:text-indigo-400 hover:underline inline-flex items-center gap-0.5 mt-2"
                >
                  Get GPS Driving Directions <ChevronRight size={12} />
                </a>
              </div>
            </div>
          </div>
